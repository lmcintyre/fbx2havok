...
//    anim->m_annotationTracks.setSize(numTracks);
    ...
    for (int y = 0; y < numTracks; y++) {
        ...
        const char *CurrentBoneName = skeleton->m_bones[currentBoneIndex].m_name;
...
//        auto aParams = new hkaSplineCompressedAnimation::AnimationCompressionParams();
        ...
        auto outAnim = new hkaSplineCompressedAnimation(*anim);
        newBinding->m_animation = outAnim;

        // copy transform track to bone indices
        for (int t = 0; t < numTracks; t++)
            newBinding->m_transformTrackToBoneIndices.pushBack(bindings[stackNum]->m_transformTrackToBoneIndices[t]);
